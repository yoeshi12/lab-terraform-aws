locals {
  ...
  # Tomamos las dos primeras zonas de disponibilidad de la región
  azs = slice(data.aws_availability_zones.disponibles.names, 0, 2)

  # Subnets públicas: 10.0.0.0/24 y 10.0.1.0/24
  subnets_publicas = {
    for i, az in local.azs : az => cidrsubnet(var.vpc_cidr, 8, i)
  }

  # Subnets privadas: 10.0.10.0/24 y 10.0.11.0/24
  subnets_privadas = {
